# toolsets/apify/instagram/fetcher.py
"""Instagram post fetcher — Protocol-based, swappable implementation."""
...
class HttpxInstagramFetcher:
    ...
    async def _fetch_graphql(
        ...
        try:
            r = await client.post(
                ...
                content=(
                    f"doc_id=10015901848480474&variables=%7B%22shortcode%22%3A%22{shortcode}%22%7D"
                ),
    ...
    async def _parse_legacy_item(self, client: httpx.AsyncClient, item: dict) -> InstagramPost:
...
class ApifyInstagramFetcher:
    """Reliable fetcher via Apify instagram-scraper actor.

    Uses Apify residential proxies — bypasses Instagram's IP blocks.
    Returns post caption + up to 20 comments with @usernames.
    Cost: ~$0.03/post (flat per run).
    """
    ...
    async def fetch(self, shortcode: str) -> InstagramPost:
        ...
        return await self._parse_item(result.items[0])
