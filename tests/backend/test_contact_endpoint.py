...
def test_contact_invalid_email(client):
    res = client.post("/v2/contact", json={"name": "Test", "email": "not-an-email", "message": "Hello"})
    assert res.status_code == 422


def test_contact_name_too_long(client):
    res = client.post("/v2/contact", json={"name": "x" * 201, "email": "test@example.com", "message": "Hello"})
    assert res.status_code == 422


def test_contact_message_too_long(client):
    res = client.post("/v2/contact", json={"name": "Test", "email": "test@example.com", "message": "x" * 4001})
    assert res.status_code == 422


def test_contact_no_api_key(client, monkeypatch):
    monkeypatch.delenv("RESEND_API_KEY", raising=False)
    res = client.post("/v2/contact", json={"name": "Jan", "email": "jan@example.com", "message": "Test"})
