...
_DOMAIN_ALL = DomainConfig(
    ...
    tiktok_search=True, instagram_search=True,
    facebook_search=True, reddit_search=True,
)
...
_IG = EmbedCandidate(url="https://instagram.com/reel/abc/", title="Reel", source="instagram",
...
async def test_reddit_included_when_flag_on():
    with (
        ...
        patch("agents.media_search.agent.search_images", new_callable=AsyncMock, return_value=[]),
        patch("agents.media_search.agent.search_reddit", new_callable=AsyncMock, return_value=[_RD]),
    ):
