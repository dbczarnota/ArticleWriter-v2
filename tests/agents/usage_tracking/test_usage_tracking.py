...
from agents.usage_tracking.agent import run_usage_tracking_agent, _UsageOutput
...
_EXTRACTION = ExtractionResult(
    facts=[Fact(text="Fact one happened", context="ctx", source_url="https://a.com", source_title="A")],
...
def test_returns_used_facts_and_quotes():
    mock_agent = Agent(
        TestModel(custom_output_args={"used_facts": ["Fact one happened"], "used_quotes": ["hello world"]}),
        output_type=_UsageOutput,
    )
    used_facts, used_quotes = asyncio.run(
        run_usage_tracking_agent(_ARTICLE, extraction_result=_EXTRACTION, config=_CONFIG, _agent=mock_agent)
    )
