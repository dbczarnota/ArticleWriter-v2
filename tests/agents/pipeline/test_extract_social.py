...
from agents.pipeline._helpers import extract_social_from_search
...
def test_non_social_url_stays_scrapable():
    ...
    scrapable, embeds = extract_social_from_search(results)
...
def test_www_youtube_com_becomes_embed():
    ...
    _, embeds = extract_social_from_search(results)
    assert embeds[0].source == "youtube"
...
def test_empty_snippet_gives_none_description():
    ...
    _, embeds = extract_social_from_search([result])
...
def test_empty_input_returns_empty():
    scrapable, embeds = extract_social_from_search([])
    assert scrapable == []
    assert embeds == []
