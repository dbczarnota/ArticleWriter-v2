...
def setup_fallback_model(
    ...
    if not models:  # Handles if models is None or an empty list []
        ...
        logger.info(
            f"No specific models provided (or list was empty) for FallbackModel setup. Using internal defaults: {internal_default_models}"
        )

    logger.debug("\n[bold blue]--- FallbackModel Initialization Start ---[/bold blue]")
    logger.debug(
        f"Attempting to initialize FallbackModel with: {models_to_actually_use}"
    )
    ...
    need_openrouter = any(
        model in openrouter_models for model in models_to_actually_use
    )
        ...
        if effective_openai_api_key:
            try:
                ...
                openai_provider = OpenAIProvider(
                    api_key=effective_openai_api_key, http_client=custom_http_client
                )
                logger.debug("OpenAI Provider initialized successfully.")
            except Exception as e:
                logger.warning(
                    f"Failed to initialize OpenAI Provider: {e}", exc_info=True
                )
        else:
            logger.warning(
                "OPENAI_API_KEY not found in args or env. Skipping OpenAI provider initialization."
            )
    else:
    ...
    if need_gemini:
        ...
        if effective_gemini_api_key:
            try:
                ...
                logger.warning(
                    f"Failed to initialize Google GLA Provider: {e}", exc_info=True
                )
        else:
            logger.warning(
                "GEMINI_API_KEY not found in args or env. Skipping Gemini provider initialization."
            )
    else:
    ...
    if need_groq:
        ...
        if effective_groq_api_key:
            try:
                ...
                logger.warning(
                    f"Failed to initialize Groq Provider: {e}", exc_info=True
                )
        else:
            logger.warning(
                "GROQ_API_KEY not found in args or env. Skipping Groq provider initialization."
            )
    else:
    ...
    if need_openrouter:
        effective_openrouter_api_key = openrouter_api_key or os.getenv(
            "OPENROUTER_API_KEY"
        )
        if effective_openrouter_api_key:
            ...
            logger.warning(
                "OPENROUTER_API_KEY not found in args or env. Skipping Openrouter provider initialization."
            )
    else:
    ...
    if need_ollama:
        ...
        effective_ollama_base_url = (
            ollama_base_url or "https://z8dc1gdrcy9i17.proxy.runpod.net/v1"
        )
        try:
        ...
        if initializer is None:
            ...
            try:
                ...
                if model_instance:
                    ...
                    logger.warning(
                        f"Provider not available for model '{model_str}'. Skipping it."
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to initialize model '{model_str}': {e}", exc_info=True
                )
...
if __name__ == "__main__":
    ...
    agent = Agent(
        ...
        system_prompt="Concisely answer the user prompt in user prompt language",
    )
