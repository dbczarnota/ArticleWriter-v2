...
if TYPE_CHECKING:
    from agents._base.types import ParsedArticle, ScrapedPage, SearchResult
...
class PipelineLogger:
    def __init__(self, enabled: bool = True) -> None:
        ...
        if enabled:
            ...
            utf8_out = open(sys.stdout.fileno(), mode="w", encoding="utf-8", closefd=False)  # noqa: SIM115 — Console owns the handle for the process lifetime
    ...
    def _err(self, stage: str, exc: BaseException) -> None:
    ...
    def extraction_done(self, result: ExtractionResult) -> None:
        ...
        self._ok(
            f"{len(result.facts)} facts   {len(result.quotes)} quotes   "
    ...
    def instructions_done(self, brief: WritingBrief) -> None:
        ...
        self._ok(
            f"{len(brief.selected_facts)} facts selected   "
        ...
        self._c.print(
            f"  [bold]Writing instructions:[/] {self._head(brief.writing_instructions, 300)}"
        )
